...
SELECT x.ordA, x.ordB, y.ordD, y.ordF, z.ordC
FROM algo3 as x INNER JOIN algo3v1 as y ON (substr(x.ordA, 3,1) = substr(y.ordD, 1,1)
&& substr(x.ordB, 3,1) = substr(y.ordF, 1, 1)) INNER JOIN ordliste as z ON 
...
&& CONCAT(substr(x.ordB, 2, 1), "_", substr(y.ordD, 2, 1) IN (select ordliste from nyListe)
